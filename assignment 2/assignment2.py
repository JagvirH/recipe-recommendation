...
from sklearn.neighbors import NearestNeighbors7
...
class recomendation:
    ...
    def tastyOrNot(self):
        ...
        filtered_data = self.df.loc[self.df['rating_val'] > 10].copy() #amount of rating reviews are above 10
        filtered_data['tasty_rating'] = filtered_data['rating_avg'].apply(lambda x: 1 if x > 4.2 else -1) #Binary conversion

        #Build the model
        X = filtered_data.drop(['tasty_rating', 'id', 'title', 'image_url', 'recipe_url', 'category', 'cuisine', 'ingredients'], axis=1)
        y = filtered_data['tasty_rating']
        ...
        #Conclude the accuracy of the model
        accuracy = accuracy_score(y_test, y_pred)
        print(f"Accuracy: {accuracy:.2f}")

        return lr

        #The accuracy of the model is influenced by several factors such as the quality and size of the dataset, or the chosen alogorithm to
        #train the model. Through several iterations using different random_states the accuracy scales around 95-99%. Which is very accurate.
...
print("---------------------------Top 10 Best Rated Recipes---------------------------")
...
'''
I put the data on the chart and their are no strong correlations accept 
...
# df.columns = [col.strip() for col in df.columns] '''Just in case it doesnt work'''
check1.tastyOrNot()
